...
- Motyw: przejście z pomarańczowego brutalizmu na profesjonalny zielono-biało-grafitowy (theme.ts: brandPrimary #2F6B4F, onSurface grafit #1F2933, tła #F4F6F8, borderStrong zmiękczony do #D3D9DF; radius 8/12/16/pill; dodano `shadow`). ui.tsx: zaokrąglone karty/przyciski/inputy, miękkie cienie, lekkie obramowania. Bottom tab active = zielony.
- Edytor kosztorysu przebudowany na zakładki: Podsumowanie / Pozycje / Zdjęcia / Ustawienia. Tytuł jako karta z pill statusu. Nowoczesny dolny panel (Zapisz + Generuj PDF). Kosztorys wewnętrzny (koszty/marża/zysk) przeniesiony do zakładki Ustawienia.
- Tryb kalkulacji (calc_mode, zapisywany w kosztorysie): labor_materials (domyślny) / labor_only (materiały widoczne, NIE liczone) / labor_selected_materials (checkbox „licz materiał" na pozycji materiału; tylko included_in_calc liczone). Robocizna/extra zawsze liczone. Przełączenie NIE usuwa pozycji; kwota przelicza się live; PDF respektuje tryb (nieliczone materiały poza tabelą i sumą).
- Backend: material_counts() w compute_totals + _counts() w pdf_service; EstimateItem.included_in_calc; EstimateIn/UpdateIn calc_mode; PUT persystuje calc_mode + image_paths. estimate_out setdefault calc_mode.
- Podgląd zdjęć: PhotoViewer (react-native-gesture-handler + reanimated, pinch-zoom/double-tap/pan, swipe między zdjęciami, licznik, usuwanie). Miniatury w Podsumowaniu (tap→viewer) i grid w zakładce Zdjęcia. Dodawanie z aparatu/galerii (expo-image-picker, normalizacja JPEG, upload przez uploadFile) i usuwanie — z auto-zapisem image_paths do kosztorysu. UWAGA: pełne działanie aparatu/galerii i gestów zoomu tylko na urządzeniu/Expo Go (nie w web preview).
- Testy: backend 14/14 nowych (test_calc_mode_and_photos.py) + 161/164 (3 to znany flake xdist w test_estimate_pipeline, niezwiązany). Frontend zweryfikowany wizualnie: zakładki, live przeliczenie trybu (4501,65→806,27 przy labor_only), regresje OK.
- Pliki: backend/server.py, backend/pdf_service.py; frontend/src/theme.ts, frontend/src/components/ui.tsx, frontend/src/components/photo-viewer.tsx (NOWY), frontend/app/estimate/[id].tsx, frontend/app/capture.tsx, frontend/app/(tabs)/_layout.tsx.
